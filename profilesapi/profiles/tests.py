...
class RegistrationTestCase(APITestCase):
  ...
  def test_registration(self):
    data = {"username": "testcase", "email": "localhost@email.com",
            "password1": "somestrongpsw", "password2": "somestrongpsw"}
...
class ProfileViewSetTestCase(APITestCase):
  ...
  def test_profile_update_by_owner(self):
    response = self.client.put(reverse("profile-detail", kwargs={"pk": 1}),
                                {"city": "Anchiano", "bio": "Reneissance Genius"})
    self.assertEqual(response.status_code, status.HTTP_200_OK)
    self.assertEqual(json.loads(response.content), {"id": 1, "user": "davinci",
                    "bio": "Reneissance Genius", "city": "Anchiano", "avatar": None})

  def test_profile_update_by_random_user(self):
    random_user = User.objects.create_user(username="random", password="psw123123123")
    self.client.force_authenticate(user=random_user)
    response = self.client.put(reverse("profile-detail", kwargs={"pk": 1}),
                                {"city": "Chelentano", "bio": "Hacked by Genius"})
    self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class ProfileStatusViewSetTestCase(APITestCase):
  ...
  def setUp(self):
    ...
    self.status = ProfileStatus.objects.create(user_profile=self.user.profile,
                              status_content="status test via automated test")
    self.token = Token.objects.create(user=self.user)
    self.api_authentication()
  ...
  def test_status_list_authentication(self):
    response = self.client.get(self.list_url)
    self.assertEqual(response.status_code, status.HTTP_200_OK)

  def test_status_list_un_authentication(self):
    self.client.force_authenticate(user=None)
    response = self.client.get(self.list_url)
    self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
  ...
  def test_status_update_owner(self):
    data = {"status_content": "content was updated"}
    response = self.client.put(reverse("status-detail", kwargs={"pk": 1}),
                                                data=data)
    self.assertEqual(response.status_code, status.HTTP_200_OK)
    self.assertEqual(response.data["status_content"], "content was updated")
